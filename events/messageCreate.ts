import { ChannelType, Events, Message, PermissionFlagsBits, TextChannel } from "discord.js";
...
export async function execute(message: Message) {
  ...
  if (message.channel.id === "1515789716291719340") {
    ...
    if (
      ...
      await (message.client.channels.cache.get("1001166932407496754") as TextChannel).send({
        embeds: [
          {
            color: 0xf04a47,
  ...
  if (message.mentions.has(process.env.CLIENT_ID!)) {
    ...
    if (!question && !hasImages) {
      ...
      const typingInterval = setInterval(() => {
        (message.channel as unknown as TextChannel).sendTyping().catch(() => {});
      }, 7000);
